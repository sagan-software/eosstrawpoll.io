...
public:
    ...
    fc::variant get_stats(const string& symbolname)
    {
        ...
        return data.empty() ? fc::variant()
                            : abi_ser.binary_to_variant("stats", data);
    }
    ...
    fc::variant get_account(account_name acc, const string& symbolname)
    {
        ...
        return data.empty() ? fc::variant()
                            : abi_ser.binary_to_variant("account", data);
    }
    ...
    fc::variant get_token(account_name acc, id_type token_id)
    {
        ...
        return data.empty() ? fc::variant()
                            : abi_ser.binary_to_variant("token", data);
    }

    action_result create(account_name issuer, string symbol)
    {
        return push_action(
            N(eosio.nft), N(create), mvo()("issuer", issuer)("symbol", symbol));
    }
        ...
        string memo)
    {
        return push_action(
            issuer,
            N(issue),
            mvo()("to", to)("quantity", quantity)("uris", uris)("memo", memo));
    }
    ...
    {
        return push_action(
            from, N(transfer), mvo()("from", from)("to", to)("id", id)("memo", memo));
    }

    action_result burn(account_name owner, id_type token_id)
    {
        return push_action(
            owner, N(burn), mvo()("owner", owner)("token_id", token_id));
    }
...
BOOST_FIXTURE_TEST_CASE(create_tests, nft_tester)
try {
    auto token = create(N(alice), string("NFT"));
    auto stats = get_stats("0,NFT");
    REQUIRE_MATCHING_OBJECT(stats, mvo()("supply", "0 NFT")("issuer", "alice"));
    produce_blocks(1);
}
FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE(symbol_already_exists, nft_tester)
try {
    auto token = create(N(alice), string("NFT"));
    auto stats = get_stats("0,NFT");
    REQUIRE_MATCHING_OBJECT(stats, mvo()("supply", "0 NFT")("issuer", "alice"));
...
BOOST_FIXTURE_TEST_CASE(issue_multi_tests, nft_tester)
try {
    ...
    REQUIRE_MATCHING_OBJECT(stats, mvo()("supply", "5 TKN")("issuer", "alice"));
    ...
    for (auto i = 0; i < 5; i++) {
        ...
        REQUIRE_MATCHING_OBJECT(
            tokenval,
            mvo()("id", i)("uri", uris[i])("owner", "alice")("value", "1 TKN"));
    }

    auto alice_balance = get_account(N(alice), "0,TKN");
    REQUIRE_MATCHING_OBJECT(alice_balance, mvo()("balance", "5 TKN"));
}
FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE(issue_tests, nft_tester)
try {
    auto newtoken = create(N(alice), string("TKN"));
    produce_blocks(1);
    ...
    auto stats = get_stats("0,TKN");
    REQUIRE_MATCHING_OBJECT(stats, mvo()("supply", "1 TKN")("issuer", "alice"));

    auto tokenval = get_token(N(alice), 0);
    REQUIRE_MATCHING_OBJECT(
        tokenval,
        mvo()("id", "0")("uri", "uri")("owner", "alice")("value", "1 TKN"));

    auto alice_balance = get_account(N(alice), "0,TKN");
    REQUIRE_MATCHING_OBJECT(alice_balance, mvo()("balance", "1 TKN"));

    BOOST_REQUIRE_EQUAL(
        wasm_assert_msg("to account does not exist"),
        issue(N(alice), N(dummy), asset::from_string("1 TKN"), uris, "hola"));

    BOOST_REQUIRE_EQUAL(
        wasm_assert_msg("quantity must be a whole number"),
    ...
    BOOST_REQUIRE_EQUAL(
        wasm_assert_msg("memo has more than 256 bytes"),
    ...
    BOOST_REQUIRE_EQUAL(
        wasm_assert_msg("must issue positive quantity of NFTs"),
        issue(N(alice), N(alice), asset::from_string("-1 TKN"), uris, "hole"));

    BOOST_REQUIRE_EQUAL(
        wasm_assert_msg("mismatch between number of tokens and uris provided"),
...
BOOST_FIXTURE_TEST_CASE(transfer_tests, nft_tester)
try {
    auto token = create(N(alice), string("NFT"));
    produce_blocks(1);
    ...
    REQUIRE_MATCHING_OBJECT(alice_balance, mvo()("balance", "1 NFT"));

    auto bob_balance = get_account(N(bob), "0,NFT");
    REQUIRE_MATCHING_OBJECT(bob_balance, mvo()("balance", "2 NFT"));

    auto tokenval = get_token(N(bob), 1);
    REQUIRE_MATCHING_OBJECT(
        tokenval,
        mvo()("id", "1")("uri", "uri2")("owner", "bob")("value", "1 NFT"));

    transfer(N(bob), N(carol), 1, "send token 1 to carol");

    auto bob_balance1 = get_account(N(bob), "0,NFT");
    REQUIRE_MATCHING_OBJECT(bob_balance1, mvo()("balance", "1 NFT"));

    auto carol_balance = get_account(N(carol), "0,NFT");
    REQUIRE_MATCHING_OBJECT(carol_balance, mvo()("balance", "1 NFT"));

    auto tokenval_1 = get_token(N(carol), 1);
    REQUIRE_MATCHING_OBJECT(
        tokenval_1,
        mvo()("id", "1")("uri", "uri2")("owner", "carol")("value", "1 NFT"));
    ...
    BOOST_REQUIRE_EQUAL(
        wasm_assert_msg("sender does not own token with specified ID"),
...
BOOST_FIXTURE_TEST_CASE(burn_tests, nft_tester)
try {
    auto token = create(N(alice), string("NFT"));
    ...
    issue(
        N(alice), N(alice), asset::from_string("2 NFT"), uris, "issue 2 tokens");

    burn(N(alice), 1);

    auto stats = get_stats("0,NFT");
    REQUIRE_MATCHING_OBJECT(stats, mvo()("supply", "1 NFT")("issuer", "alice"));
    ...
    REQUIRE_MATCHING_OBJECT(stats2, mvo()("supply", "0 NFT")("issuer", "alice"));
}
FC_LOG_AND_RETHROW()
